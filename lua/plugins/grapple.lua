...
---
---TODO: (High) Move function setup to own grapple config file post-lazy

local M = {}

function M.grapple_open_index(n)
  local cmd_grapple = function()
    vim.cmd('Grapple select index=' .. tostring(n))
  end
  local grapple = require 'grapple'
  if grapple.tags { index = n } then
    return cmd_grapple
  else
    return function()
      vim.notify('Grapple index ' .. n .. ' does not exist', vim.log.levels.WARN)
    end
  end
end
...
M.plugins = {
  {
    ...
    keys = {
      ...
      -- { ';q', M.grapple_open_index(1) }, -- testing
